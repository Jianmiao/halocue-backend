...
class Legacy093Adapter:
    ...
    def create_performance_draft(
        ...
        if self.settings.resource_index:
            ...
            try:
                source_resources = json.loads(self.settings.resource_index.read_text(encoding="utf-8"))
            ...
            if database.is_file():
                ...
                try:
                    ...
                    source_resources = self._modules["asset_catalog"].merge_model_constraints(
                        source_resources,
                        connection,
                        scope=str(self.store.get_draft_path(token)),
                    )
            ...
            resource_path.write_text(
                json.dumps(
                    self.name_baseline.decorate_resource_payload(source_resources),
                    ensure_ascii=False,
                    indent=2,
                ),
                encoding="utf-8",
    ...
    def direction_proposals(self, token: str) -> dict[str, Any]:
        ...
        detail = self.draft_detail(token)
        current_revision = detail.get("content_revision")
        current_cards = {
        ...
        for attempt_dir in sorted((item for item in root.iterdir() if item.is_dir()), reverse=True):
            ...
            for proposal in proposals if isinstance(proposals, list) else []:
                ...
                application_mode = str(
                    proposal.get("application_mode") or "legacy_applied"
                )
                card = current_cards.get(safe_card_id)
                current = card.get("current") if isinstance(card, dict) else {}
                expected_value = (
                    proposal.get("before")
                    if application_mode == "proposal_only"
                ...
                if safe:
                    reason = (
                        "已唯一对应到当前草稿中的一张台词。采纳后才会写入建议值；拒绝不会修改草稿。"
                        if application_mode == "proposal_only"
                        else "这是旧版已写入建议。你可以保留该值，或撤销并恢复生成前的值。"
                    )
            ...
            generations.append(
                {
                    ...
                    "proposal_only": bool(result.get("proposal_only")),
        ...
        *, proposal: dict[str, Any], source_cards: list[dict[str, Any]], result_cards: list[dict[str, Any]]
        ...
        for card in result_cards:
            ...
            if len(source) != 1 or str(source[0].get("current", {}).get(field) or "").strip() != before:
                continue
            matches.append(str(source[0].get("card_id") or ""))
    ...
    def _anchor_direction_proposals(
        ...
        *,
        ...
        nodes = self.document.normalize_draft_nodes(
            self.document.parse_document_lossless(result_text)
        )
        result_cards = [
            {
                "card_id": "",
        ...
        for original in proposals:
            ...
            proposal["application_mode"] = "proposal_only"
    ...
    def decide_direction_proposal(
        ...
        """Accept a proposal-only value or handle a legacy pre-applied proposal."""
        if action not in {"approve", "reject"}:
            raise ProductionError("invalid_proposal_action", "只能采纳或拒绝 AI 演出建议")
        draft_dir = self.store.get_draft_path(token)
        with self.store.draft_lock(token):
            ...
            if not card or str((card.get("current") or {}).get(field) or "").strip() != str(expected_current or "").strip():
                raise ProductionError("proposal_stale", "对应台词已经被修改；这条建议不再适用", status=409)
            should_write = (
                application_mode == "proposal_only" and action == "approve"
            ) or (
                application_mode != "proposal_only" and action == "reject"
            )
            if should_write:
                value = (
                    proposal.get("after")
                    if application_mode == "proposal_only"
                    else proposal.get("before")
                )
                self.update_card(
                    token=token, card_id=card_id,
                    patch={field: str(value or "")},
                    expected_draft_version=expected_draft_version,
                )
            proposal["state"] = "approved" if action == "approve" else "rejected"
            proposal["decision"] = "accepted" if action == "approve" else "rejected"
    ...
    def execute_direction_generation(
        ...
        *,
        ...
        draft_dir = self.store.get_draft_path(token)
        source_detail = self.draft_detail(token)
        if source_detail.get("draft_version") != expected_draft_version:
            raise ProductionError("revision_conflict", "草稿版本已经变化", status=409)
        source_cards = source_detail.get("cards") or []
        source_content_revision = int(source_detail.get("content_revision") or 0)
        ...
        # The annotated file is Attempt evidence only. It cannot become the
        # current draft until a user accepts an anchored Proposal.
        anchored_proposals = self._anchor_direction_proposals(
            proposals=result.get("proposals") or [],
            source_cards=source_cards,
            result_text=str(result["text"]),
            content_revision=source_content_revision,
        )
        proposals_file = attempt_dir / "proposals.json"
        proposals_temporary = proposals_file.with_suffix(".json.tmp")
        proposals_temporary.write_text(
            json.dumps(anchored_proposals, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        os.replace(proposals_temporary, proposals_file)
        summary = {
        ...
        result_file = attempt_dir / "result.json"
        result_temporary = result_file.with_suffix(".json.tmp")
        result_temporary.write_text(
            json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        os.replace(result_temporary, result_file)
        return summary
