...
def test_ai_direction_background_job_creates_proposals_without_writing_draft(settings, tmp_path, monkeypatch):
    configured = configured_resource_settings(settings, tmp_path)
    monkeypatch.setenv("HALOCUE_TEST_MODEL_KEY", "test-secret")
    service = ProductionService(configured)
    service.configure_direction_model(
    ...
    assert job["result"]["proposal_only"] is True
    detail = service.run_detail(created["run"]["run_id"])
    assert detail["run"]["state"] == "waiting_for_review"
    assert detail["draft"]["draft_version"] == mapped["draft"]["draft_version"]
    assert detail["draft"]["content_revision"] == mapped["draft"]["content_revision"]
    attempt = (
        configured.data_dir
        / "drafts"
...
def test_proposal_only_change_is_written_only_after_acceptance(settings):
    service = ProductionService(settings)
    created = service.create_run(
        {"project": "AI Proposal 采纳", "source": {"kind": "inline", "text": "旁白: 原句\n"}}
    )
    run_id = created["run"]["run_id"]
    draft = created["draft"]
    line = draft["cards"][0]
    attempt = service.adapter.store.get_draft_path(created["run"]["draft_token"]) / "direction-generations" / "direction-proposal-only"
    attempt.mkdir(parents=True)
    (attempt / "result.json").write_text(
        json.dumps({"generation_id": "direction-proposal-only", "model": "test", "draft_version": draft["draft_version"], "proposal_only": True}),
        encoding="utf-8",
    )
    proposals = [
        {
            "proposal_id": "prop-reject-only",
            ...
            "after": "惊讶",
            "state": "pending",
            "safe_card_id": line["card_id"],
            "based_on_content_revision": draft["content_revision"],
        },
        {
            "proposal_id": "prop-accept-only",
            "type": "applied_pending",
            "application_mode": "proposal_only",
            "field": "face",
            "before": None,
    ...
    (attempt / "proposals.json").write_text(
        json.dumps(proposals, ensure_ascii=False), encoding="utf-8"
    )

    audit = service.direction_proposals(run_id)
    assert all(item["can_apply_safely"] for item in audit["generations"][0]["proposals"])
    assert all(
        item["application_mode"] == "proposal_only"
        for item in audit["generations"][0]["proposals"]
    )

    rejected = service.decide_direction_proposal(
        run_id,
        "prop-reject-only",
        {"action": "reject", "expected_draft_version": draft["draft_version"]},
    )
    rejected_line = next(
        item for item in rejected["draft"]["cards"] if item["card_id"] == line["card_id"]
    )
    assert rejected["draft"]["draft_version"] == draft["draft_version"]
    assert not rejected_line["current"].get("emo")

    accepted = service.decide_direction_proposal(
        run_id,
        "prop-accept-only",
        {"action": "approve", "expected_draft_version": draft["draft_version"]},
    )
    accepted_line = next(
        item for item in accepted["draft"]["cards"] if item["card_id"] == line["card_id"]
    )
    assert accepted["draft"]["draft_version"] == draft["draft_version"] + 1
    assert accepted_line["current"]["face"] == "00"
    decisions = {
        item["proposal_id"]: item["state"]
        for item in service.direction_proposals(run_id)["generations"][0]["proposals"]
    }
    assert decisions == {
        "prop-reject-only": "rejected",
        "prop-accept-only": "approved",
    }
    service.jobs.close()
...
def test_aa_workspace_configuration_persists_and_enables_capabilities(
    ...
    assert str(aa_data.resolve()) not in json.dumps(configured, ensure_ascii=False)
...
def test_real_compile_and_install_use_isolated_workspace(settings, tmp_path):
    ...
    configured = Settings(
        ...
        data_dir=settings.data_dir,
        legacy_root=settings.legacy_root,
        resource_index=settings.legacy_root / "aa_resources.json",
        aa_data=aa_data,
        host="127.0.0.1",
        port=0,
    )
    ...
    assert (aa_data / "projects" / "测试分类-隔离构建测试.aap").is_file()
    assert str(aa_data.resolve()) not in json.dumps(installed, ensure_ascii=False)
    service.jobs.close()
...
def test_aa_environment_can_inspect_and_adopt_data_workspace(settings, tmp_path):
    ...
    assert inspected["environment"]["workspace"]["discovered"] is True
    assert str(aa_data.resolve()) not in json.dumps(inspected, ensure_ascii=False)
    ...
    assert str(aa_data.resolve()) not in json.dumps(adopted, ensure_ascii=False)
    service.jobs.close()
