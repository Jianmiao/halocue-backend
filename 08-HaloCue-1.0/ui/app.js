(() => {
  ...
  async function startGeneration() {
    ...
    try {
      const result = await api(`/production-runs/${state.currentRun.run_id}/direction-generation`, {
        method: "POST", body: JSON.stringify({ expected_draft_version: state.currentDraft.draft_version, story_type: "auto" })
      });
  ...
  function renderDirectionProposals(audit) {
    ...
    target.className = "";
    target.innerHTML = `<section class="proposal-summary"><strong>共 ${audit.total} 条 AI 演出建议</strong><p>模型建议不会自动写入草稿。只有能唯一对应到当前台词、且草稿仍与生成依据一致的建议才能采纳。</p></section>${generations.map((generation) => `<section class="proposal-generation"><header><div><h4>一次生成记录 · ${esc(generation.proposal_count)} 条建议</h4><small>${esc(generation.model || "未记录模型")} · 剧情类型：${esc(generation.story_type || "auto")} · 基于草稿版本：${esc(generation.draft_version || "-")}</small></div></header><ul class="proposal-list">${(generation.proposals || []).map((proposal) => {
      const suggested = proposal.type === "suggested_fix";
      const proposalOnly = proposal.application_mode === "proposal_only";
      const before = proposal.before || "未设置";
      const after = proposal.after || "未设置";
      let action = "";
      if (proposal.can_apply_safely) {
        action = proposalOnly
          ? `<div class="proposal-actions"><button type="button" data-proposal-action="approve" data-proposal-mode="proposal_only" data-proposal-id="${esc(proposal.proposal_id)}">采纳建议</button><button type="button" class="danger" data-proposal-action="reject" data-proposal-mode="proposal_only" data-proposal-id="${esc(proposal.proposal_id)}">拒绝建议</button></div>`
          : `<div class="proposal-actions"><button type="button" data-proposal-action="approve" data-proposal-mode="legacy_applied" data-proposal-id="${esc(proposal.proposal_id)}">保留旧版标注</button><button type="button" class="danger" data-proposal-action="reject" data-proposal-mode="legacy_applied" data-proposal-id="${esc(proposal.proposal_id)}">撤销并恢复原值</button></div>`;
      }
      const description = suggested
        ? "模型提出了这个值，但它只作为参考记录。"
        : proposalOnly
          ? "这个建议尚未写入草稿；只有明确采纳后才会更新对应台词。"
          : "这是旧版流程已写入的标注，可保留或撤销。";
      const badge = suggested ? "仅供参考" : proposal.can_apply_safely ? proposalOnly ? "待采纳" : "旧版待确认" : proposal.state === "pending" ? "已失效" : "已决定";
      return `<li class="proposal-item ${suggested ? "suggested" : "applied"}"><div><strong>${esc(proposalFieldLabel(proposal.field))}：${esc(after)}</strong><p>${description}</p><div class="proposal-change"><span>原值：${esc(before)}</span><span>建议值：${esc(after)}</span></div><p>${esc(proposal.apply_reason)}</p>${action}</div><b>${badge}</b></li>`;
    }).join("")}</ul></section>`).join("")}`;
    status.textContent = `当前草稿为第 ${audit.draft_version || "-"} 版；本窗口不会修改它。`;
    $$('[data-proposal-action]').forEach((button) => button.addEventListener("click", () => decideDirectionProposal(button.dataset.proposalId, button.dataset.proposalAction, button.dataset.proposalMode)));
  }

  async function decideDirectionProposal(proposalId, action, applicationMode) {
    if (!state.currentRun || !state.currentDraft) return;
    const proposalOnly = applicationMode === "proposal_only";
    const verb = proposalOnly
      ? action === "approve" ? "采纳这项 AI 建议" : "拒绝这项 AI 建议"
      : action === "reject" ? "撤销并恢复生成前的值" : "保留这项旧版 AI 标注";
    const effect = proposalOnly
      ? action === "approve" ? "这会更新对应台词，并让该卡回到待审。" : "这只记录拒绝决定，不会修改草稿。"
      : action === "reject" ? "这会修改对应台词，并让该卡回到待审。" : "这只记录你的确认，不会改写台词。";
    if (!window.confirm(`${verb}？${effect}`)) return;
    try {
      ...
      applyRun(result);
      toast(proposalOnly ? action === "approve" ? "已采纳 AI 建议；请审查这张卡。" : "已拒绝 AI 建议，草稿未修改。" : action === "reject" ? "已撤销旧版 AI 标注。" : "已保留旧版 AI 标注。");
  ...
  function renderAaEnvironment(result) {
    ...
    const workspaceStatus = workspace.discovered ? "已发现本地 AA 工作区" : "未找到工作区";
    const issue = environment.issues?.[0];
    status.className = `environment-status ${workspace.valid ? "ready" : "needs-work"}`;
    status.innerHTML = `<strong>${workspace.valid ? "AA 制作环境可用" : "还不能用于制作"}</strong><p>${esc(workspaceStatus)}</p><div><span>工程目录 ${workspace.directories?.projects ? "可用" : "缺失"}</span><span>存档目录 ${workspace.directories?.saves ? "可用" : "缺失"}</span><span>官方资源 ${cache.available ? "已发现" : "未发现"}</span></div>${issue ? `<small>${esc(issue.code)} · ${esc(issue.message)}</small>` : ""}`;
